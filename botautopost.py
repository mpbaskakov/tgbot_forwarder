...
def send_document(bot, job):
    ...
    print(job.context + ' 0', file=open('log.txt', 'a'))
    if not file_list:
        pass
    else:
        file_id = file_list[randint(0, len(file_list) - 1)][0]
        print(job.context + ' 1', file=open('log.txt', 'a'))
        if id == 0:
            print(job.context + ' 2', file=open('log.txt', 'a'))
            if ad_counter % 3 == 0:
                ...
                print(job.context + ' 3', file=open('log.txt', 'a'))
            else:
                bot.send_document(chat_name, file_id)
                print(job.context + ' 4', file=open('log.txt', 'a'))
            ad_counter += 1
            print(job.context + ' 5', file=open('log.txt', 'a'))
            if ad_counter == 20000: ad_counter = 0
        else:
            bot.send_document(chat_name, file_id)
        write_to_base(chat_name[1:], file_id, erase=True)
        print(job.context + ' 6', file=open('log.txt', 'a'))
    id += 1
    print(job.context + ' 7', file=open('log.txt', 'a'))
    if id == 4: id = 0
    print(job.context + ' 8', file=open('log.txt', 'a'))
